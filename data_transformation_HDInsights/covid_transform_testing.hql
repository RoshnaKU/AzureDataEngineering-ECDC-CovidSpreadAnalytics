...
CREATE EXTERNAL TABLE IF NOT EXISTS covid_reporting_lookup.dim_date (
   ...
   STORED AS TEXTFILE
   LOCATION 'abfs://lookup@ecdcstore.dfs.core.windows.net/dim_date/'
...
CREATE EXTERNAL TABLE IF NOT EXISTS covid_reporting_lookup.dim_country (
   ...
   ROW FORMAT DELIMITED FIELDS TERMINATED BY ','
   STORED AS TEXTFILE
   LOCATION 'abfs://lookup@ecdcstore.dfs.core.windows.net/dim_country/'
   TBLPROPERTIES ("skip.header.line.count"="1");
...
CREATE EXTERNAL TABLE IF NOT EXISTS covid_reporting_raw.testing (
   ...
   ROW FORMAT DELIMITED FIELDS TERMINATED BY ','
   STORED AS TEXTFILE
   LOCATION 'abfs://raw@ecdcstore.dfs.core.windows.net/testing/'
   TBLPROPERTIES ("skip.header.line.count"="1");
...
CREATE TABLE IF NOT EXISTS covid_reporting_processed.testing (
   ...
   ROW FORMAT DELIMITED FIELDS TERMINATED BY ','
   STORED AS TEXTFILE
   LOCATION 'abfs://processed@ecdcstore.dfs.core.windows.net/testing'
   TBLPROPERTIES ("skip.header.line.count"="1");
